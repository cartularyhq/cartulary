...
- HTTP endpoint tests for health, ingest, search, ask, context, and knowledge.
- Database tests for POC account header behavior, scope inheritance, message
  persistence, knowledge insertion, lifecycle event insertion, and deterministic
  extractor fallback.
- Eval fixture baselines committed for the tiny Cartulary, LoCoMo, LongMemEval,
  and BEAM fixtures already described in `docs/poc-local-proof.md`.
- A short transition note in `Cartulary.Memory` naming it as a temporary facade
  until Ash actions replace it.

Acceptance:

- Existing POC commands still pass.
- Tests fail if an agent can write knowledge directly or if request body data
