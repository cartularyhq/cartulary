...
today lives in `docs/implementation-status.md`.

This roadmap merges and replaces three earlier documents:

- `docs/roadmap/l3-automation-flow.md` (delivery workflow),
- `docs/roadmap/main-branch-ruleset.md` (branch protection checklist),
- `docs/roadmap/manual-automation-setup.md` (maintainer setup runbook),

and it absorbs the still-open phases of the former
`docs/roadmap/free-core-roadmap.md`, whose durable architecture content now
lives at `docs/architecture/free-core-architecture.md`.
...
Delivered and covered by tests, documented in `docs/implementation-status.md`
and the notes under `docs/architecture/`:
...
`docs/eval/surface-contract-inventory.json` records generated OpenAPI and the
...
- [ ] Flip `ash_json_api_openapi` in
  `docs/eval/surface-contract-inventory.json` from `unavailable` to `gated`
  with real evidence paths.

### 1.2 MCP tool completion

- [ ] Audit the shipped `ash_ai` MCP tool set against the public operation set
  in `docs/architecture/free-core-architecture.md` and close the gaps.
...
- [ ] Write down and enforce the versioning rule for API paths, OpenAPI
  ...
  packages in `docs/operations/versioning.md`.
...
- [ ] Confirm the required set matches
  `docs/operations/release-checklist.md`; update both together if a job is
  renamed.
...
- [ ] Document every external service in `docs/security/` or `docs/eval/`
...
- [ ] Retire `Cartulary.Memory` as a facade once every surface calls Ash
  ...
  `docs/architecture/free-core-architecture.md`.
- [ ] Leave historical artefacts alone and say so in the PR:
  ...
  recorded in `docs/eval/results/`. These are immutable evidence.
...
- Target architecture: `docs/architecture/free-core-architecture.md`.
- What runs today: `docs/implementation-status.md`.
