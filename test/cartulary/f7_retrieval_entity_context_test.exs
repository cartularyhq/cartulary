...
defmodule Cartulary.F7RetrievalEntityContextTest.CardSummaryConcurrencyProvider do
  @moduledoc """
...
defmodule Cartulary.F7RetrievalEntityContextTest do
  ...
  test "entity card summaries overlap up to the configured concurrency" do
    ...
    original_provider = Application.get_env(:cartulary, :model_provider)
    original_concurrency = Application.get_env(:cartulary, :entity_card_summary_concurrency)

    on_exit(fn ->
      Cartulary.F7RetrievalEntityContextTest.CardSummaryConcurrencyProvider.stop()
      Application.put_env(:cartulary, :entity_card_summary_concurrency, original_concurrency)
    ...
    Application.put_env(
      :cartulary,
      :model_provider,
      Cartulary.F7RetrievalEntityContextTest.CardSummaryConcurrencyProvider
    )
    ...
    Application.put_env(:cartulary, :entity_card_summary_concurrency, 1)
    Cartulary.F7RetrievalEntityContextTest.CardSummaryConcurrencyProvider.start!()
    ...
    assert Cartulary.F7RetrievalEntityContextTest.CardSummaryConcurrencyProvider.peak() == 1

    Application.put_env(:cartulary, :entity_card_summary_concurrency, 2)
    Cartulary.F7RetrievalEntityContextTest.CardSummaryConcurrencyProvider.start!()

    assert {:ok, %{entity_cards: 2, entity_card_summaries_unavailable: 0}} =
             Builder.refresh_scope(ledger.account.id, ledger.scope.id)

    assert Cartulary.F7RetrievalEntityContextTest.CardSummaryConcurrencyProvider.peak() == 2
