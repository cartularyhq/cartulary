...
defmodule Cartulary.Identity do
  ...
  The credential kind is decided by its shape: generated API keys always begin
  with `cartulary_`, so that prefix routes to the API-key path and anything else
  binary is treated as a session token. This is why the key prefix is part of
  the wire contract and cannot be changed casually — every deployed client's
  credential would stop being recognised as a key.
