...
defense-in-depth. `Cartulary.DataLayer`'s moduledoc calls this "two independent
...
superuser's. Every connection Cartulary made was a superuser connection:
...
in every deployment mode.** `Cartulary.Database.AppRole` creates a role —
...
supervised boot steps carry this out, added to `Cartulary.Application`'s child
list in the position each needs:

1. `Cartulary.Database.RoleProvisioner` creates and grants the role, before the
   ...
   switched to.
2. The repository's `:after_connect` callback (`Cartulary.Database.AppRole.set_role/1`)
...
3. `Cartulary.Database.RoleGuard` runs after migrations and before anything
...
`Cartulary.Database.AppRole.with_privileged_repo/1`, which starts a second
...
(`Cartulary.Release.migrate/0`) already ran outside the supervised application
boot and were never affected; the supervised migration step
(`Cartulary.Release.Migrator`, used by the pg0 turnkey path) now explicitly
...
restricted role. `test/cartulary/database/app_role_test.exs` asserts the
running connection's own `pg_roles` attributes. A new assertion in
`test/cartulary/f1_ash_domain_backbone_test.exs` seeds a row, clears every
...
hand-written query in `Cartulary.Retrieval.Store` is caught by the database
...
The Compose path connects as `cartulary`, created via the official Postgres
