...
> - **ADR-0009 — scope-bounded entity cards.** A private `entity_id` coordinate keys one projection per scope and resolved entity. Cards use at least three distinct active sources, carry their strictest sensitivity, and expose only a bounded summary plus governed statement fields. Adds **AD-DATA-11** and `FR-KN-23`; retrieval candidates remain deferred pending evaluation.
>
...
- **AD-DATA-5 — Vector + lexical co-located; two embedding collections.** Both indices live with storage so index writes ride the knowledge-write transaction (AD-SEAM-4.4): `ash_ai` `vectorize` + pgvector + PG-FTS over one connection, in every mode (ADR-0003). The index is durable and transactional — there is no rebuildable in-memory variant and no boot reconciliation. Two logical collections: knowledge-statement embeddings (for retrieving knowledge) and document-chunk embeddings (for `search`, FR-API-7). Both feed the FR-API-25 fusion stage — since ADR-0004 as two of N `Retrieval.Strategy` modules (`Semantic`, `Lexical`) rather than one hybrid implementation. Co-location is also why fan-out is cheap: every strategy is a concurrent query on the same connection pool, not a scatter-gather across heterogeneous stores.
...
- **AD-DATA-11 — Entity cards are scope-bounded projections, not entity content (ADR-0009, FR-KN-23).** `Projection.entity_id` is a nullable, private cache coordinate; the Account-local key is `entity:<scope_id>:<entity_id>`. The visible payload contains a bounded summary, its model provenance, the strictest source sensitivity, and allowlisted governed statement fields. It contains no entity-cache field. Cards use distinct active statements only and require at least three sources, so provisional subject-only knowledge cannot enter a subjectless aggregate and long-tail single mentions cause no summary call. Current core reads authorize sensitivity at Gate B and then scope at read time; there is no independent reader-clearance axis to invent here. Persisting the strictest sensitivity keeps the aggregate correctly classified and is the policy filter point if sensitivity-scoped field reads are introduced later. Lifecycle invalidation remains scope-wide, and rebuilds leave cards below threshold dirty.
...
- **AD-CFG-2 — Packaging & distribution.** *Rewritten by ADR-0003.* The primary on-ramp is **one command with zero dependencies and no container runtime**: a self-contained Mix release plus a **pinned pg0 binary** (PostgreSQL 18 + pgvector, MIT, fully offline), where the release supervises the pg0 process as a child, runs migrations on boot, and serves. Nothing else must be installed — no Docker, no Postgres, no Erlang. Data lives in the pg0 instance directory; `pg0` platform support (macOS ARM64/x86_64, Linux Debian-Ubuntu x86_64/ARM64, Linux Alpine 3.20–3.21, Windows x64) is therefore Cartulary's local-install matrix. **Three distribution paths, one release:**
