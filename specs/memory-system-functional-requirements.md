...
- **FR-PLAT-2** **Single-node mode (free):** one BEAM node running everything, against a **local PostgreSQL** with **pgvector** and **PG-FTS**, with **Oban (Postgres engine)** for jobs and background workers. Limited to a **single Account**. The local Postgres shall be provisioned by an **embedded, pinned `pg0` binary supervised by the release**, so that first run requires **one command and no pre-installed dependencies — no Docker, no separate Postgres install** (ARCH `AD-CFG-2`, ADR-0003). Pointing the same release at an externally managed Postgres shall remain available as a connection-string change.
...
- **FR-PLAT-4** The **vector layer**, **lexical layer**, **data layer**, and **job-queue** shall be **identical in every deployment mode** (pgvector, PG-FTS, AshPostgres, Oban-Postgres). They shall remain behind interfaces so the implementation is replaceable, but **no second backend is maintained**, and no deployment mode may be served by a different storage, index, or queue implementation than the one exercised by the test suite (ADR-0003; supersedes the earlier two-backends-per-interface requirement).
- **FR-PLAT-5** Self-host shall be distributed as a **single Mix release** by three paths, all from the same build: (a) **no-container** — release + pinned `pg0`, one command, zero dependencies (the default local on-ramp, FR-PLAT-2); (b) **container image + compose files** for single-node and queue mode, which shall use **stock Postgres, not `pg0`** (`pg0` refuses to run as root); (c) **external Postgres** via connection string. Deployment mode is selected via runtime config, not by a different build.
