# Cartulary Enterprise License

Copyright (c) 2026-present Aleksei Popov.

This license governs Cartulary code and documentation whose path contains
`/ee/`, whose filename contains `.ee.`, or whose file header identifies
`SPDX-License-Identifier: Cartulary-Enterprise`.

You may view, copy, and modify enterprise-marked software for development,
testing, security review, and evaluation. Production use, commercial use,
distribution, hosting, resale, or use of enterprise features requires a valid
Cartulary Enterprise license, valid entitlements for the features used, or
another written commercial agreement with the licensor.

Modifications, patches, and derivative works of enterprise-marked software are
governed by this same license. No right is granted to distribute, sublicense,
sell, rent, offer as a service, or otherwise exploit enterprise-marked software
except as expressly permitted by a valid enterprise license or written commercial
agreement.

Enterprise-marked software is source-available so customers and evaluators can
inspect, integrate, and test it. The visibility of source code is not permission
to bypass, remove, disable, or misrepresent license or entitlement checks.

Third-party components incorporated into enterprise-marked software remain under
the licenses supplied by their respective owners.

THE SOFTWARE IS PROVIDED "AS IS", WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND,
INCLUDING IMPLIED WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR
PURPOSE, TITLE, AND NON-INFRINGEMENT. TO THE MAXIMUM EXTENT PERMITTED BY LAW,
THE LICENSOR IS NOT LIABLE FOR DAMAGES ARISING FROM THE SOFTWARE OR THIS
LICENSE.
